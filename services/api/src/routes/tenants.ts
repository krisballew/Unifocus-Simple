...
export async function tenantRoutes(server: FastifyInstance) {
  ...
  server.get(
    '/api/tenants/:id/settings',
    {
      schema: {
        tags: ['tenants'],
        description: 'Get tenant locale and scheduling settings',
        params: z.object({
          id: z.string(),
        }),
        response: {
  ...
  server.patch(
    '/api/tenants/:id/settings',
    {
      schema: {
        tags: ['tenants'],
        description: 'Update tenant locale and scheduling settings (admin only)',
        params: z.object({
          id: z.string(),
        }),
        body: {
    ...
    async (request, reply) => {
      ...
      if (!authCtx.scopes.includes('tenant:admin')) {
        return reply.status(403).send({
          code: 'FORBIDDEN',
      ...
      const { weekStartDay, defaultLocale, defaultTimezone, defaultCurrency } = request.body as {
        weekStartDay?: number;
